...
export const useAuth = () => {
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      const user: any = await $fetch(
        'https://ecoommerce.runasp.net/api/Account/login', {
          method: 'POST',
          body: { email, password },
        }
      )
  ...
  const register = async (fName: string, lName: string, uName: string, email: string, password: string) => {
    ...
    try {
      ...
      await $fetch(
        'https://ecoommerce.runasp.net/api/Account/register', {
          method: 'POST',
          body,
        }
      )
  ...
  const signInWithGoogle = async (response: any) => {
    ...
    try {
      const user = await $fetch<AuthResponse>(
        'https://ecoommerce.runasp.net/api/Account/signin-google', {
        method: 'POST',
        body: { idToken }
      })
  ...
  const refresh = async () => {
    ...
    try {
      const { accessToken: at, refreshToken } = await $fetch<AuthResponse>(
        'https://ecoommerce.runasp.net/api/Account/refresh-token',
  ...
  const forgotPassword = async (email: string) => {
    ...
    try {
      const result = await $fetch(
        'https://ecoommerce.runasp.net/api/Account/forgot-password', {
          method: 'POST',
          body: {
            email,
  ...
  const resetPassword = async (email: string, token: string, newPassword: string) => {
    ...
    try {
      await $fetch(
        'https://ecoommerce.runasp.net/api/Account/reset-password', {
          method: 'POST',
          body,
        }
    )
  ...
  const logout = async () => {
    ...
    try {
      loading.value = true
      await $fetch('https://ecoommerce.runasp.net/api/Account/logout', {
        method: 'POST',
        body: { refreshToken },
        headers: {
  ...
  const send_otp = async (email: string) => {
    try {
      loading.value = true
      await $fetch(`https://ecoommerce.runasp.net/api/Account/send-otp`, {
        method: 'POST',
        params: { email }
      })
    } catch (err: any) {
  ...
  const confirm_email = async (email: string, otp: string) => {
    try {
      loading.value = true
      const user = await $fetch<AuthResponse>('https://ecoommerce.runasp.net/api/Account/confirm-email', {
        method: 'POST',
        body: { email, code: otp },
      })
      authStore.setTokens(user.accessToken, user.refreshToken, user)
  ...
  const users_count = async () => {
    try {
      ...
      const users = await useAsyncData('getAllUsers',async () => 
        await $fetch('https://ecoommerce.runasp.net/api/Account/users-count', {
        method: 'POST'
        })
      )
      return users
